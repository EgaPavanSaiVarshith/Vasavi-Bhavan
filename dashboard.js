...
var App = window.App || {};
App.Dashboard = {
    render: function () { this._metrics(); this._birthdays(); this._anniversaries(); this._upcoming(); this._recent(); },

    _metrics: function () {
        this._count('metricTotal', App.DB.getApproved().length);
        this._count('metricPending', App.DB.getPending().length);
        this._count('metricBirthdays', App.DB.getTodayBirthdays().length);
        this._count('metricAnniversaries', App.DB.getTodayAnniversaries().length);
    },
    ...
    _birthdays: function () {
        var c = document.getElementById('dashBirthdays'), b = App.DB.getTodayBirthdays();
    ...
    _anniversaries: function () {
        var c = document.getElementById('dashAnniversaries'), a = App.DB.getTodayAnniversaries();
    ...
    _upcoming: function () {
        var c = document.getElementById('dashUpcoming'), ev = App.DB.getUpcomingEvents(7);
    ...
    _recent: function () {
        var c = document.getElementById('dashRecent'), r = App.DB.getRecent(5);
